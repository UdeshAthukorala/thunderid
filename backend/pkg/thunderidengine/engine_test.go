...
func (suite *EngineTestSuite) TestNew_AppliesLogFormatToServicesBuiltInsideNew() {
	...
	t.Cleanup(func() {
		...
		// New raises the process-wide level to debug through WithLogConfig below, and the
		// level lives on the shared root, so restore the boot default for later tests.
